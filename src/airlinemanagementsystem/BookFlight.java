    ...
    public void actionPerformed(ActionEvent ae) {
        try {
            ...
            if (ae.getSource() == fetchButton) {
                ...
                String bookingRef = "PNR-" + random.nextInt(1000000);
                String ticketNo = "TIC-" + random.nextInt(10000);

                String query = "insert into reservation values('" + bookingRef + "', '" + ticketNo + "', '" + aadhar + "', '" + name
                        + "', '" + nationality + "', '" + flightname + "', '" + flightcode + "', '" + src + "', '" + des + "', '" + ddate + "')";
                conn.s.executeUpdate(query);

//                JOptionPane.showMessageDialog(null, "Ticket Booked Successfully!\nBooking Reference: " + bookingRef);

                // Automatically open DummyPayment with bookingRef and amount
                double amount = 5000.0; // you can calculate based on flight
                new DummyPayment(bookingRef, amount);
            }
        } catch (Exception e) {
            e.printStackTrace();
